...
class App:
    def __init__(self):
        pyxel.init(SCREEN_WIDTH, SCREEN_HEIGHT, display_scale=1, fps=60)
        self.random_create = 0
        self.players = []
        self.players.append(
            Player(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2, PLAYER_RADIUS, PLAYER_HP)
        )
        self.players.append(
            Player(
                SCREEN_WIDTH // 2 + PLAYER_RADIUS * 2,
                SCREEN_HEIGHT // 2,
                PLAYER_RADIUS,
                PLAYER_HP,
            )
    ...
    def update(self):
        ...
        if len(self.enemies) != 0:
            if self.rate_time > BULLET_FIRE_RATE:
                for player in self.players:
                    self.bullets.append(
                        Bullet(
                            player.x,
